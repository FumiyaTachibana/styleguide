...
- ハッシュはオブジェクトIDが同じであるがゆえ、可読性を高めたり、メモリを節約し実行速度が速くなるなどのメリットがあります。
- また、シンボルオブジェクトに代入した文字情報は不変で、その後も同一のオブジェクトを指し示すマークであり続けます。
...
##### section-4-5
### 可変パラメータ
- 可変パラメータを使用することで、複数の引数をもつメソッドに対して、引数を(配列として渡すことで)一度に引数を渡すことができます。その際は”引数”の前に「*」(アスタリスク)を付けましょう。
- また、引数の数が呼び出し毎に変わるときなどには、”最後の引数”の前に「*」を付けます。
- そして、 メソッドから別のメソッドで可変パラメータを呼ぶ場合は＊を付けないと配列が多重化してしまうので必ず引数に＊を付けましょう。
...
```ruby
# 仮に下記のコードで例外が発生し、rescueで抜けた際に処理をきちんと書いていないとエラーが発生してしまう。
def hoge
  begin
    # 例外が発生する可能性のある式
  rescue
